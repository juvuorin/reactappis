...
const Koulu=()=> {
    ...
    useEffect(() => {
        ...
        if (!dataNoudettu) {
            let result = localStorage.getItem("kouluData")
            if (result == null) {
                setKoulu(koulu_)
            } else {
                setKoulu(JSON.parse(result))
            }
 ...
 */
    return (<div>{(dataNoudettu==true) && <div>{koulu.nimi}{koulu.luokat.map((item, index) => <Luokka luokka={item} luokkaIndex={index} dispatch={dispatch} koulu={koulu} />)}
